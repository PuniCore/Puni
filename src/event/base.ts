  ...
  constructor ({
    ...
    this.reply = async (elements, options) => {
      try {
        ...
        if (this.isGroup) {
          this.#bot.adapter.protocol === AdapterProtocol.Console && logger.bot('info', this.selfId, `${logger.green(`Send Group ${this.contact.peer}: `)}${raw.replace(/\n/g, '\\n')}`)
        } else {
          this.#bot.adapter.protocol === AdapterProtocol.Console && logger.bot('info', this.selfId, `${logger.green(`Send private ${this.contact.peer}: `)}${raw.replace(/\n/g, '\\n')}`)
        }
